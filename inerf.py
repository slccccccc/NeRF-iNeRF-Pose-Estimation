...
def load_nerf(args, device):
    ...
    #ckpt = torch.load(lego_path)
    #ckpt = torch.load(c_path)
    # Load model
...
def run_inerf(_overlay=False, _debug=False):
    ...
    for k in range(60):
        ...
        if (k + 1) % 1 == 0:
            ...
            with torch.no_grad():
                ...
                phi_error = abs(phi_ref - phi) if abs(phi_ref - phi)<180 else abs(abs(phi_ref - phi)-360)
                theta_error = abs(theta_ref - theta) if abs(theta_ref - theta)<180 else abs(abs(theta_ref - theta)-360)
                psi_error = abs(psi_ref - psi) if abs(psi_ref - psi)<180 else abs(abs(psi_ref - psi)-360)
            ...
            if _overlay is True and (k + 1) % 10 == 0:
                with torch.no_grad():
            ...
            print('-----------------------------------')
        err = np.array([[k, loss.item(), rot_error, translation_error, phi, theta, psi, translation]])
        errors = np.concatenate((errors, err), axis=0)


    t = int(time())
    savetxt(f'logs//{t}_data.csv', errors, delimiter=',', fmt='%0.4f')
    #errors.tofile(f'{os.getcwd()}//logs//{t}_data.csv', sep=',', format='%0.2f')

    # ## load data and read
    training_history = np.loadtxt(f'logs//{t}_data.csv', delimiter=",", dtype=float)
    ...
    axs[0].plot(ks, losses, color='red')
    axs[0].set_title('Loss')
    axs[0].set_xlabel('k')
    axs[0].set_ylabel('Loss')

    axs[1].plot(ks, rot_errors, color='blue')
    axs[1].set_title('Rotation Error')
    axs[1].set_xlabel('k')
    axs[1].set_ylabel('Rotation Error')

    axs[2].plot(ks, translation_errors, color='green')
    axs[2].set_title('Translation Error')
    axs[2].set_xlabel('k')
